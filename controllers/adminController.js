...
//     inline_keyboard.push([
//       { text: "◀️ Admin menyu", callback_data: "back_to_admin" },
//     ]);
	...
	try {
		...
		if (users.length === 0) {
			...
			users.forEach((user, index) => {
				...
				message += `${userNumber}. ${user.fullName}\n`
				message += `   ⭐ ${user.points} | 👥 ${user.referrals} | 📅 ${joinDate} | ${status}\n\n`
			})
		}
		...
		// ✅ TO'G'RILANGAN: Pagination tugmalari faqat kerak bo'lsa ko'rsatiladi
		if (totalPages > 1) {
			const paginationButtons = []

			// ✅ 1. Oldingi sahifa faqat 1-dan katta sahifalar uchun
			if (page > 1) {
				paginationButtons.push({
					text: '⬅️ Oldingi',
					callback_data: `users_page_${page - 1}`
			...
			// ✅ 2. Joriy sahifa raqami
			paginationButtons.push({
				text: `${page}/${totalPages}`,
				callback_data: 'current_page'
			})

			// ✅ 3. Keyingi sahifa faqat oxirgi sahifa bo'lmaganda
			if (page < totalPages) {
				paginationButtons.push({
					text: 'Keyingi ➡️',
					callback_data: `users_page_${page + 1}`
			...
			// ✅ Agar kamida 2 ta tugma bo'lsa (masalan: [1] yoki [<] [1] [>])
		...
		inline_keyboard.push([{ text: "🆕 So'nggi 7 kun", callback_data: 'recent_users' }])

		inline_keyboard.push([{ text: '◀️ Admin menyu', callback_data: 'back_to_admin' }])

		// Xabar uzunligini tekshirish
		if (message.length > 4096) {
			// Agar xabar juda uzun bo'lsa, uni qismlarga bo'lish
		...
		console.error("❌ Foydalanuvchilar ro'yxatini ko'rsatish xatosi:", error)
		await bot.sendMessage(chatId, '❌ Xatolik yuz berdi')
	}
}

// Xabarni qismlarga bo'lish uchun yordamchi funksiya
function splitMessage(text, maxLength) {
	...
	for (const line of lines) {
		if (currentPart.length + line.length + 2 <= maxLength) {
    ...
    const keyboard = {
      reply_markup: {
        inline_keyboard: [
          [
            {
              text: "🔄 Kunlik bonusni o'zgartirish",
              callback_data: "change_daily_bonus",
            },
        ...
        `Yangi vaqtni tanlang:`,
      {
        ...
        reply_markup: {
          inline_keyboard: [
            ...
            [
              {
                text: "⌨️ Qoʻlda kiritish",
                callback_data: "custom_bonus_time",
              },
              { text: "❌ Bekor qilish", callback_data: "set_daily_bonus" },
            ],
          ],
  ...
  try {
    ...
    const weeklyBonusStats = await User.aggregate([
      ...
      },
      {
        $group: {
          _id: {
            $dateToString: { format: "%Y-%m-%d", date: "$lastDailyBonus" },
          },
          count: { $sum: 1 },
        },
      },
...
const handleToggleBonusStatus = async (chatId, enable) => {
  try {
    // MongoDB ga saqlash
    const success = await Settings.updateDailyBonusSettings({
      enabled: enable,
    });
    ...
    await bot.sendMessage(
      ...
      `${emoji} *Kunlik bonus ${status}!*\n\n` +
        ...
        `Foydalanuvchilar ${
          enable
            ? "endi kunlik bonus olishni boshlaydilar"
            : "endi kunlik bonus olmaydilar"
        }.`,
      {
        parse_mode: "Markdown",
        reply_markup: {
          inline_keyboard: [
            [{ text: "🔙 Orqaga", callback_data: "set_daily_bonus" }],
          ],
        },
      },
    );
  ...
  try {
    ...
    await bot.sendMessage(
        ...
        `Iltimos, vaqtni quyidagi formatda kiriting:`,
      {
        ...
        reply_markup: {
          inline_keyboard: [
            [{ text: "❌ Bekor qilish", callback_data: "set_bonus_time" }],
          ],
        },
      },
    );
...
const processBonusSettings = async (chatId, text) => {
  try {
    ...
    if (
      state &&
      state.action === "change_daily_bonus" &&
      ...
      if (isNaN(amount) || amount < 0) {
        await bot.sendMessage(
          chatId,
          "❌ Noto'g'ri miqdor. Faqat musbat raqam kiriting (0-1000):",
        );
        return;
      }
      ...
      if (amount > 1000) {
        await bot.sendMessage(
          chatId,
          "❌ Miqdor juda katta. Maksimal 1000 ball kiriting:",
        );
        return;
      }
