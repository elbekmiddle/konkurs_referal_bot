...
const handleEditFieldSelection = async (chatId, data) => {
  try {
    ...
    const fieldInstructions = {
      ...
      image:
        'Yangi rasm yuboring yoki "🚫 Rasmsiz davom etish" tugmasini bosing:',
    ...
    const currentValue =
      state.contestData[field === "winners" ? "winnersCount" : field];
            ...
            [
              {
                ...
                callback_data: "skip_edit_image",
              },
              {
                text: "❌ Bekor qilish",
                callback_data: `admin_contest_${state.contestId}`,
              },
            ],
          ],
        },
  ...
  try {
    ...
    if (state.step === "edit_image" && state.waitingForImage) {
      ...
      if (hasImage) {
        ...
        console.log(
          `⚠️ Text received instead of image in image edit: "${text}"`,
        );
