...
const showMessageReportMenu = async (chatId) => {
    try {
        ...
        const message = `📬 *ADMINGA XABAR YUBORISH*\n\n` +
...
const handleReportTypeSelection = async (chatId, type) => {
    try {
        ...
        const message = `${selectedType.emoji} *${selectedType.name.toUpperCase()} XABARI*\n\n` +
...
const processReportMessage = async (chatId, msg) => {
    try {
        ...
        if (photo && photo.length > 0) {
            ...
            await bot.sendMessage(chatId, '✅ Rasm qabul qilindi. Xabarni yozishda davom eting yoki "✅ Xabarni yuborish" tugmasini bosing.', {
                reply_markup: {
                    inline_keyboard: [
        ...
        if (document) {
            ...
            await bot.sendMessage(chatId, '✅ Fayl qabul qilindi. Xabarni yozishda davom eting yoki "✅ Xabarni yuborish" tugmasini bosing.');
            return;
        }
        ...
        for (const admin of adminUsers) {
            try {
                ...
                const keyboard = {
                    inline_keyboard: [
                        [
                            { text: '👀 Ko\'rish', callback_data: `view_report_${report._id}` },
                ...
                console.error(`Admin ${admin.chatId} ga xabar yuborish xatosi:`, adminError);
            }
        }
    } catch (error) {
    ...
    try {
        ...
        if (reports.length === 0) {
            ...
            reports.forEach((report, index) => {
                ...
                const shortMessage = report.message.length > 30 
        ...
        if (totalPages > 1) {
            ...
            if (page > 1) {
                paginationRow.push({ text: '◀️', callback_data: `reports_page_${page - 1}` });
            }
...
const showReportDetail = async (chatId, reportId) => {
    try {
        ...
        if (report.status !== 'resolved') {
            keyboard.inline_keyboard.push([
                { text: '✅ Hal qilindi', callback_data: `resolve_report_${report._id}` },
...
const startReplyToReport = async (chatId, reportId) => {
    try {
        ...
        await bot.sendMessage(chatId, `✍️ *XABARGA JAVOB YOZISH*\n\n` +
...
const processReplyMessage = async (chatId, msg) => {
    try {
        ...
        if (text && text.trim()) {
            ...
            // Javobni tasdiqlash
            await bot.sendMessage(chatId, `📋 *JAVOB TAQDIMI*\n\n` +
                ...
                reply_markup: {
                    inline_keyboard: [
                        [
                            { text: '✅ Yuborish', callback_data: `submit_reply_${state.reportId}` },
                            { text: '✏️ Qayta yozish', callback_data: `rewrite_reply_${state.reportId}` }
                        ],
                        [
                            { text: '🚫 Bekor qilish', callback_data: `cancel_reply_${state.reportId}` }
                        ]
                    ]
                }
            });
...
const submitReply = async (chatId, reportId) => {
    try {
        ...
        try {
            await bot.sendMessage(report.reporterId, 
                `📬 *ADMIN JAVOBI*\n\n` +
        ...
        // Admin ga tasdiq
        await bot.sendMessage(chatId, `✅ Javob muvaffaqiyatli yuborildi!\n\n` +
...
const resolveReport = async (chatId, reportId) => {
    try {
        ...
        await bot.sendMessage(chatId, `✅ Xabar hal qilindi!\n\n` +
...
const showReportsStats = async (chatId) => {
    try {
        const totalReports = await MessageReport.countDocuments();
        const pendingReports = await MessageReport.countDocuments({ status: 'pending' });
        const resolvedReports = await MessageReport.countDocuments({ status: 'resolved' });
        const bugReports = await MessageReport.countDocuments({ messageType: 'bug' });
        const suggestionReports = await MessageReport.countDocuments({ messageType: 'suggestion' });

        const last24Hours = new Date(Date.now() - 24 * 60 * 60 * 1000);
        const todayReports = await MessageReport.countDocuments({ createdAt: { $gte: last24Hours } });

        const message = `📊 *XABARLAR STATISTIKASI*\n\n` +
            `📈 Umumiy statistik:\n` +
