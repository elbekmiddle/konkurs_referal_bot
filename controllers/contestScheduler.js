...
class ContestScheduler {
	...
	async scheduleContest(contest) {
		try {
			...
			if (startDate > now && !contest.isActive) {
				...
				console.log(`   ⏰ Boshlanish: ${Math.floor(startDelay / 1000)} soniyadan keyin`)
			...
			if (contest.isActive && endDate > now) {
				...
				console.log(`   ⏰ Tugash: ${Math.floor(endDelay / 1000)} soniyadan keyin`)
	...
	async selectWinners(contestId) {
		try {
			...
			// G'oliblar soni
			const winnerCount = Math.min(contest.winnersCount || 1, participantsWithScores.length)
			...
			winners.forEach((winner, index) => {
				console.log(`   ${index + 1}. ${winner.fullName} - ${winner.score} ball`)
			})
	...
	async checkContests() {
		try {
			...
			const startJobs = Array.from(this.jobs.entries()).filter(([key, job]) => job.type === 'start')
			...
			const endJobs = Array.from(this.jobs.entries()).filter(([key, job]) => job.type === 'end')
