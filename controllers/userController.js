...
const processReferral = async (referrerChatId, newUser) => {
  try {
    console.log(
      `🔍 Referal qidirilmoqda: ${referrerChatId} -> ${newUser.chatId}`,
    );
    ...
    const existingReferral = referrer.referredUsers?.find(
      (ref) => ref.chatId === newUser.chatId,
    );
    ...
    console.log(
      `✅ Referal muvaffaqiyatli: ${referrer.chatId} -> ${newUser.chatId}`,
    );
    ...
    try {
      ...
      console.log(
        "⚠️ Taklif qilgan foydalanuvchiga xabar yuborishda xato:",
        error.message,
      );
    }
    ...
    try {
      ...
      console.log(
        "⚠️ Taklif qilingan foydalanuvchiga xabar yuborishda xato:",
        error.message,
      );
    }
  } catch (error) {
      ...
      if (
        ...
        if (referrer) {
          ...
          if (referrer.isSubscribed) {
            ...
            console.log(
              `✅ Darhol referal bonus berildi: ${referrer.chatId} -> ${user.chatId}`,
            );
          } else {
            console.log(
              `ℹ️ Referrer hali obuna bo'lmagan, faqat refBy ni saqlaymiz`,
            );
  ...
  try {
    ...
    if (query === "share_contest" || query.includes("konkurs")) {
      ...
      const contestDescription =
        activeContest.description || "Konkurs tavsifi mavjud emas";
      const contestReward =
        activeContest.reward || activeContest.rewardPoints || 0;
      const contestWinners =
        activeContest.winnerCount || activeContest.winnersCount || 1;
...
const handleJoinContest = async (chatId, contestId, referrerId) => {
  try {
    ...
    await bot.sendMessage(
      chatId,
      "❌ Konkursga qo'shilishda xatolik yuz berdi.",
    );
  }
};
...
const handleCheckSubscription = async (chatId) => {
  try {
    ...
    const loadingMsg = await bot.sendMessage(
      chatId,
      "🔍 Kanallarga obuna holati tekshirilmoqda...",
    );
    ...
    for (const channel of channels) {
      try {
        console.log(
          `🔍 Kanal tekshirilmoqda: ${channel.name} (ID: ${channel.channelId})`,
        );
...
const showChannelsForSubscriptionWithStatus = async (
  chatId,
  channels,
  notSubscribedChannels,
) => {
...
const handleConfirmSubscription = async (chatId) => {
  try {
    ...
    const loadingMsg = await bot.sendMessage(
      chatId,
      "🔍 Obuna holatingiz tekshirilmoqda...",
    );
  ...
  try {
    ...
    if (user.refBy && !user.hasReceivedReferralBonus) {
      ...
      if (referrer && referrer.isSubscribed) {
        ...
        const alreadyExists = referrer.referredUsers.find(
          (ref) => ref.chatId === user.chatId,
        );
        if (!alreadyExists) {
          referrer.referredUsers.push({
            chatId: user.chatId,
        ...
        console.log(
          `✅ Referal bonus berildi: ${referrer.chatId} -> ${user.chatId}`,
        );
        ...
        console.log(
          `⚠️ Referrer topilmadi yoki obuna bo'lmagan: ${user.refBy}`,
        );
      }
    } else {
...
const showLeaderboardAsTable = async (chatId) => {
  try {
    ...
    if (currentUser) {
      const userRank =
        (await User.countDocuments({ points: { $gt: currentUser.points } })) +
        1;
  ...
  try {
    ...
    if (currentFriends.length > 0) {
      ...
      currentFriends.forEach((friend, index) => {
        ...
        const date = new Date(friend.joinDate)
...
const showUserStatsAsTable = async (chatId) => {
  try {
    ...
    message += `• O'rtacha ball: ${
      user.points > 0
        ? Math.round(user.points / (user.referredUsers.length || 1))
        : 0
    }\n`;
  ...
  try {
    ...
    if (image) {
      await bot.sendPhoto(chatId, image, {
        ...
        reply_markup: {
          inline_keyboard: [
            [{ text: "🎯 Konkursga qatnashish", url: referralLink }],
          ],
        },
      });
    } else {
  ...
  if (data.startsWith("share_contest_")) {
    try {
      ...
      let shortDescription =
        contest.description || "Konkurs tavsifi mavjud emas";
      if (shortDescription.length > 400)
        shortDescription = shortDescription.substring(0, 400) + "...";
...
const handleShareContestToFriends = async (chatId, contestId) => {
  try {
    console.log(
      `📤 Konkursni do'stlarga ulashish: ${chatId}, contest: ${contestId}`,
    );
...
const shareContest = async (chatId, contestId) => {
  try {
    console.log(
      `🔗 Konkursni ulashish (faqat matn): chatId=${chatId}, contestId=${contestId}`,
    );
...
const sendContestToChat = async (senderChatId, targetChatId) => {
  try {
    console.log(
      `📤 Konkurs postini yuborish (faqat matn): ${senderChatId} -> ${targetChatId}`,
    );
    ...
    await bot.sendMessage(
      senderChatId,
      "❌ Konkurs postini yuborishda xatolik yuz berdi.",
    );
    return null;
  }
};
...
const handleCallback = async (chatId, callbackData) => {
  try {
    ...
    if (callbackData.startsWith("share_image_")) {
      console.log(
        `ℹ️ Rasmli ulashish callback'iga kerak yo'q: ${callbackData}`,
      );
      await bot.sendMessage(
        chatId,
    ...
    switch (callbackData) {
      ...
      default:
        ...
        await bot.sendMessage(
          chatId,
          "❌ Noma'lum amal. Iltimos, boshqa tugmani bosing.",
        );
    }
  } catch (error) {
...
const checkSubscriptionRealTime = async (chatId) => {
  try {
    ...
    console.log(
      `📋 Real-time tekshiriladigan kanallar soni: ${channels.length}`,
    );
...
const shareContestWithImageDesign = async (chatId, contestId) => {
  try {
    console.log(
      `🎯 Konkursni rasmdagi dizaynda ulashish: chatId=${chatId}, contestId=${contestId}`,
    );
