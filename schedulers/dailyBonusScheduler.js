...
const distributeDailyBonuses = async () => {
	try {
		...
		console.log(`📊 ${usersToUpdate.length} ta foydalanuvchi bonus olishi kerak`)
		...
		for (const user of usersToUpdate) {
			try {
				...
				try {
					...
					console.log(`⚠️ Xabar yuborish xatosi (${user.chatId}):`, botError.message)
				}
			} catch (userError) {
				console.error(`❌ Foydalanuvchini yangilash xatosi (${user.chatId}):`, userError)
			}
		}

		console.log(`✅ ${successCount} ta foydalanuvchiga ${amount} ball bonus berildi`)
...
const rescheduleBonusCron = async () => {
	try {
		...
		if (settings.time && settings.time !== '00:00') {
			...
			console.log(`🔄 Bonus vaqti sozlandi: har kuni soat ${settings.time} (${cronExpression})`)
