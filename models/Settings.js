	...
	try {
		...
		if (updates.amount !== undefined) {
			settingsToUpdate.push(
				this.setSetting('daily_bonus_amount', updates.amount, 'Kunlik bonus miqdori')
			)
		}

		if (updates.enabled !== undefined) {
			settingsToUpdate.push(
				this.setSetting('daily_bonus_enabled', updates.enabled, 'Kunlik bonus holati')
			)
		}

		if (updates.time !== undefined) {
			settingsToUpdate.push(this.setSetting('daily_bonus_time', updates.time, 'Kunlik bonus vaqti'))
		}
...
settingsSchema.statics.initializeDefaults = async function () {
	const defaults = [
		{ key: 'daily_bonus_amount', value: 10, description: 'Kunlik bonus miqdori (ball)' },
		{ key: 'daily_bonus_enabled', value: true, description: 'Kunlik bonus tizimi faolmi?' },
		{ key: 'daily_bonus_time', value: '00:00', description: 'Kunlik bonus beriladigan vaqt' },
		{ key: 'referral_bonus', value: 5, description: 'Har bir referal uchun bonus ballari' },
		{ key: 'contest_join_bonus', value: 3, description: "Konkursga qo'shilish uchun bonus" },
		{ key: 'min_withdrawal', value: 100, description: 'Minimal yechib olish miqdori' },
		{ key: 'max_referrals_per_day', value: 10, description: 'Kuniga maksimal referal soni' },
		{ key: 'bonus_cooldown_hours', value: 24, description: "Bonus o'rtasidagi kutish vaqti (soat)" }
	]
